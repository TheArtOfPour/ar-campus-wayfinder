...
AFRAME.registerComponent('poi-marker', {
  ...
  init() {
    console.log('[POI Marker] Creating marker for:', this.data.name, 'at lat:', this.data.lat, 'lng:', this.data.lng);
  ...
  buildMarker() {
    ...
    console.log('[POI Marker] Building marker hierarchy');
    ...
    // 1. Teardrop marker body (smaller, more visible)
    const teardropBody = document.createElement('a-entity');
    teardropBody.setAttribute('geometry', 'primitive: sphere; radius: 0.3; segment-width: 24; segment-height: 16');
    ...
    console.log('[POI Marker] Added teardrop body');

    // 2. White border (back-face culling to show only from outside)
    const border = document.createElement('a-entity');
    border.setAttribute('geometry', 'primitive: sphere; radius: 0.35; segment-width: 24; segment-height: 16');
    border.setAttribute('material', 'color: #ffffff; side: back');
    border.setAttribute('scale', '1 2 1');
    el.appendChild(border);
    
    console.log('[POI Marker] Added white border');

    // 3. Top pointed tip
    const tip = document.createElement('a-entity');
    tip.setAttribute('geometry', 'primitive: cone; radius: 0.15; height: 0.4; segments: 8');
    tip.setAttribute('material', 'color: #667eea');
    tip.setAttribute('position', '0 1.0 0'); // Positioned at top of teardrop
    el.appendChild(tip);
    
    console.log('[POI Marker] Added tip');
    ...
    backplane.setAttribute('width', '2.5');
    backplane.setAttribute('height', '0.6');
    backplane.setAttribute('color', '#ffffff');
    backplane.setAttribute('position', '0 1.4 0');
    ...
    labelText.setAttribute('width', '2.2');
    labelText.setAttribute('position', '0 1.4 0.02'); // Slightly in front of backplane
    ...
    console.log('[POI Marker] Added text label container');
    console.log('[POI Marker] Marker creation complete');
